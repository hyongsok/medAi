...
def enneaception(pretrained_inception_dict=None, **kwargs):
    if pretrained_inception_dict is None:
        ...
        net.inception_xs.load_state_dict(pretrained_inception_dict)
        return net
...
class Enneaception(nn.Module):
    ...
    def forward(self, x):
        ...
        img2 = x[:,:,598:,:299]
        img3 = x[:,:,:299,299:598]
        img4 = x[:,:,299:598,299:598]
        img5 = x[:,:,598:,299:598]
        img6 = x[:,:,:299,598:]
        img7 = x[:,:,299:598,598:]
        img8 = x[:,:,598:,598:]

        img0 = self.inception_xs(img0)
        img1 = self.inception_xs(img1)
        img2 = self.inception_xs(img2)
        img3 = self.inception_xs(img3)
        img4 = self.inception_xs(img4)
        img5 = self.inception_xs(img5)
        img6 = self.inception_xs(img6)
        img7 = self.inception_xs(img7)
        img8 = self.inception_xs(img8)
        # 9 of n x 1000
